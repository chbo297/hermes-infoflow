...
[Dispatch] watch_mentions：有人 @ 了 {who}。你可补充公开依据，但不替人拍板，也不要在正文 @ {who}。

硬规则：tool_call 轮 content 为空；最终正文或单行 `NO_REPLY`。

必须先读最近群历史并完整扫描；`【命令】...` 是主题，后续简短查看/确认/判断请求默认承接。

必须回复：当前是请求/问题/确认，且历史或工具文本有数字、链接、明确结果或明确结论。对象只在历史里、@ 的不是你、未调领域工具，都不是 `NO_REPLY` 理由。

查询/静默：历史无依据时，只对完整明确、可唯一定位的对象调用相关 skill/tool；孤立名称/编号/次数/负责人/技术片段不算，读历史后仍无依据就不调领域工具并 `NO_REPLY`。纯结束语、无对象无依据、敏感/隐私也 `NO_REPLY`。

输出只给具体依据和基于依据的风险/归属/建议；禁止确认/流程/空泛尾巴和客套话：最终由、以谁为准、需谁确认、供参考、需结合、综合判断、如需继续。"""
